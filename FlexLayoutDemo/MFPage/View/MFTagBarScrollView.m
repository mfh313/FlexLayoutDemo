...
#import "MFPageTagView.h"

@interface MFTagBarScrollView () <UIScrollViewDelegate>

@property (strong, nonatomic) NSMutableArray <MFPageTagView *> *tagViewsCache;

@property (strong, nonatomic) UIView *markView;

@property (assign, nonatomic) NSInteger index;

@property (nonatomic, weak, readwrite) id<MFTabDataSource> tabDataSource;

@property (nonatomic, weak, readwrite) id<MFTabDelegate> tabDelegate;


@end

@implementation MFTagBarScrollView
...
- (void)__setupTagBarView
{
    ...
    for (int i=0;i<[self.tabDataSource numberOfTab];i++) {
        ...
        MFPageTagTitleView *titleView = [[MFPageTagTitleView alloc] initWithFrame:CGRectMake(offset, top, tagWidth, self.frame.size.height)];
...
- (void)markViewToIndex:(NSInteger)index animatied:(BOOL)animated
{
    ...
    if (![self needMarkView]) {
        ...
        MFPageTagView *nextTagView = self.tagViewsCache[index];
        if (animated) {
            __weak MFTagBarScrollView *wScrollView =self;
            [UIView animateWithDuration:0.3 animations:^{
                __weak MFTagBarScrollView *bScrollView =wScrollView;
...
- (void)reloadHighlight
{
    for (int i=0;i<self.tagViewsCache.count;i++) {
        MFPageTagView *view = (MFPageTagView *)self.tagViewsCache[i];
        if (i == self.index) {
...
- (void)scrollTagToIndex:(NSUInteger)toIndex
{
    ...
    MFPageTagView *nextTagView = self.tagViewsCache[toIndex];
...
- (void)markViewScrollToContentRatio:(CGFloat)contentRatio
{
    ...
    MFPageTagView *curTagView = self.tagViewsCache[fromIndex];
    MFPageTagView *nextTagView = self.tagViewsCache[fromIndex+1];


    MFPageTagView *firstTagView = self.tagViewsCache.firstObject;
    MFPageTagView *lastTagView = self.tagViewsCache.lastObject;
...
- (void)markViewScrollToIndex:(NSInteger)index
{
    ...
    MFPageTagView *curTagView = self.tagViewsCache[index];
...
{
    for (int i=0;i<[self.tabDataSource numberOfTab];i++) {
        MFPageTagTitleView *titleView = (MFPageTagTitleView *)self.tagViewsCache[i];
        if ([self.tabDataSource respondsToSelector:@selector(titleColorForIndex:)]) {
            titleView.normalTitleColor = [self.tabDataSource titleColorForIndex:i];
        }
        if ([self.tabDataSource respondsToSelector:@selector(titleHighlightColorForIndex:)]) {
