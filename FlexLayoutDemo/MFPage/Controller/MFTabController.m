...
#import "MFTagBarScrollView.h"
#import "MFPageController.h"

#define SCREEN_WIDTH    [UIScreen mainScreen].bounds.size.width
#define SCREEN_HEIGHT   [UIScreen mainScreen].bounds.size.height

@interface MFTabController () <MFPageControllerDelegate, MFPageControllerDataSource, MFTabDataSource, MFTabDelegate>

@property (nonatomic, strong) UIView *tabView;

@property (nonatomic, strong) MFPageController *pageController;
...
@implementation MFTabController
...
- (void)pageviewControllerDidTransitiontoViewController:(UIViewController *)toVC fromVC:(UIViewController *)fromVC
{
    ...
    if ([fromVC conformsToProtocol:@protocol(MFPageSubControllerDataSource) ]) {
        UIViewController<MFPageSubControllerDataSource> *fromVCTemp =  (UIViewController<MFPageSubControllerDataSource> *)fromVC;
        fromVCTemp.preferScrollView.scrollsToTop = NO;
    }

    if ([toVC conformsToProtocol:@protocol(MFPageSubControllerDataSource) ]) {
        UIViewController<MFPageSubControllerDataSource> *toVCTemp =  (UIViewController<MFPageSubControllerDataSource> *)toVC;
...
- (void)__setupPage
{
    self.pageController = [[MFPageController alloc] init];
...
- (void)__setupTabBar
{
    ...
    if (!tabView) {
        tabView = [[MFTagBarScrollView alloc] initWithFrame:[self preferTabFrame] dataSource:self delegate:self];
...
{
    if ([self.tabView isKindOfClass:[MFTagBarScrollView class]]) {
        MFTagBarScrollView *tabView = (MFTagBarScrollView *)self.tabView;
...
#pragma MFPageControllerDelegate
...
-(void)scrollViewContentOffsetWithRatio:(CGFloat)ratio draging:(BOOL)draging
{
    if ([self.tabView isKindOfClass:[MFTagBarScrollView class]]) {
        ...
        if (!draging) {
            __weak MFTabController *wSelf = self;
            [UIView animateWithDuration:0.3 animations:^{
                MFTabController *bSelf = wSelf;
                MFTagBarScrollView *scrollView = (MFTagBarScrollView *)bSelf.tabView;
            ...
            MFTagBarScrollView *scrollView = (MFTagBarScrollView *)self.tabView;
...
#pragma MFPageControllerDataSource
...
#pragma MFTabDataSource 规范化的TAB设计样式可以放在这里
