...
#import "MFPageContentView.h"

typedef NS_ENUM(NSInteger,MFPageScrollDirection) {
    PageLeft = 0,
    PageRight = 1,
};


@interface MFPageController  () <NSCacheDelegate, UIScrollViewDelegate>
...
@property (nonatomic, strong) MFPageContentView *scrollView;
...
@implementation MFPageController
...
{
     self.scrollView = [[MFPageContentView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height)];
    ...
    if (self.scrollView.frame.size.width > 0 && self.scrollView.contentSize.width > 0) {
        ...
        if (animated) {
            ...
            if (self.lastSelectedIndex != self.currentPageIndex) {//如果非交互切换的index和currentindex一样，则什么都不做
                ...
                MFPageScrollDirection direction = (self.lastSelectedIndex < self.currentPageIndex) ? PageRight :PageLeft;
                ...
                __weak MFPageController *wSelf = self;
...
- (UIViewController *)controllerAtIndex:(NSInteger)index
{
    if (![self.memCacheDic objectForKey:@(index)]) {
        ...
        if (controller) {
            ...
            if ([controller conformsToProtocol:@protocol(MFPageSubControllerDataSource)]) {
                [self bindController:(UIViewController<MFPageSubControllerDataSource> *)controller index:index];
            }
...
- (void)clearObserver
{
    for (NSNumber *key in self.memCacheDic) {
        ...
        if ([vc conformsToProtocol:@protocol(MFPageSubControllerDataSource)]) {
            UIScrollView *scrollView = [(UIViewController<MFPageSubControllerDataSource> *)vc preferScrollView];
...
- (void)bindController:(UIViewController<MFPageSubControllerDataSource> *)controller index:(NSInteger)index
{
...
- (void)resizePageAtIndex:(NSInteger)index offset:(CGFloat)offset isNeedChangeOffset:(BOOL)isNeedChangeOffset atBeginOffsetChangeOffset:(BOOL)atBeginOffsetChangeOffset
{
    ...
    if (vc && [vc conformsToProtocol:@protocol(MFPageSubControllerDataSource)]) {
        UIScrollView *scrollView = [(UIViewController<MFPageSubControllerDataSource> *)vc preferScrollView];
